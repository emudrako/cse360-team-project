	...
	public static String checkForValidUserName(String input) {
		...
		while (running) {
			...
			case 1:
				...
				// . & - _ -> State 2
						...
						currentChar == '-' || currentChar == '_') {		// Check for special chars
					nextState = 2;
		...
		case 1:
			...
			if (userNameSize < 4) {
				...
				userNameRecognizerErrorMessage +=
					"A UserName may only contain letters (A-Z, a-z), digits (0-9), or the special characters: & - _ .\n";
				return userNameRecognizerErrorMessage;
			}
			else {
					// UserName is valid
		...
		case 2:
			...
			userNameRecognizerErrorMessage +=
				"A UserName may not end with a special character (&, -, _, .); a letter or digit must follow.\n";
			return userNameRecognizerErrorMessage;
