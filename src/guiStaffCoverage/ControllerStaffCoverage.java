 ...
 *  satisfying STORY 5: Answer-Coverage Computation. Contains the core
 *  coverage computation logic (computeCoverage) and the threshold check
 *  (meetsThreshold). These methods are also called directly by
 *  CoverageReportTests to validate correctness without launching JavaFX.
...
public class ControllerStaffCoverage {
	...
	/**********
	 * <p> Method: ControllerStaffCoverage() </p>
	 *
	 * <p> Description: Default constructor — not used. This controller is a
	 *  collection of static methods and is never instantiated. </p>
	 *
	 */
	 ...
	 *
	 * <p> Description: Called when staff navigates to this screen from Staff
	 *  Home. Displays the corresponding View. Satisfies Story 5. </p>
	 *
	...
	/**********
	 * <p> Method: computeCoverage(List&lt;Reply&gt; replies, List&lt;Post&gt; posts) </p>
	 *
	 * <p> Description: Computes the deduplicated peer-reply count for every
	 *  student who has made at least one reply, satisfying STORY 5: Answer-Coverage
	 *  Computation. The filter applies in order:
	 *  (1) Skip replies where the target post is not found — guards against
	 *  orphaned reply records in the database.
	 *  (2) Skip self-replies — a student replying to their own post must not
	 *  count toward their peer threshold, satisfying AC5.4.
	 *  (3) Add the post author to a per-student Set of distinct peers. The Set
	 *  automatically deduplicates, satisfying AC5.3.
	 ...
	 *
	 * <p> Validated by: CoverageReportTests.testStudentMeetsThresholdExactly(),
	 *  CoverageReportTests.testStudentExceedsThreshold(),
	 *  CoverageReportTests.testStudentBelowThreshold(),
	 *  CoverageReportTests.testStudentHasZeroReplies(),
	 *  CoverageReportTests.testDeduplicationMultipleRepliesToSamePeer(),
	 *  CoverageReportTests.testSelfReplyExclusion(),
	 *  CoverageReportTests.testReportCompleteness(),
	 *  CoverageReportTests.testCountOfOne(),
	 *  CoverageReportTests.testLargeInput(). </p>
	 *
	...
	public static Map<String, Integer> computeCoverage(
		...
		// Return the empty map — caller handles zero-reply students via getOrDefault.
		...
		for (Reply reply : replies) {
			...
			//     Orphaned replies must not crash the computation.
			...
			//     own posts toward their peer threshold. Without this check,
			//     students could game the requirement by replying to themselves.
			...
			//     avoiding a separate null-check on every iteration.
	...
	/**********
	 * <p> Method: meetsThreshold(int peerCount) </p>
	 *
	 * <p> Description: Returns true if the given peer count meets or exceeds
	 *  PEER_THRESHOLD, satisfying STORY 5 AC5.2. Extracted into its own method
	 *  so the threshold comparison is defined in exactly one place — if the
	 *  threshold changes, only PEER_THRESHOLD needs updating. </p>
	 *
	 * <p> Validated by: CoverageReportTests.testStudentMeetsThresholdExactly(),
	 *  CoverageReportTests.testStudentBelowThreshold(),
	 *  CoverageReportTests.testCountOfOne(). </p>
	 *
	 * @param peerCount the deduplicated peer count for a student
	 *
	 * @return true if peerCount >= PEER_THRESHOLD, false otherwise
	 *
	 */
